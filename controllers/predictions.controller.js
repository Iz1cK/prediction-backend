...
const WCPredictionsModel = require("../models/wcpredictions.model");
...
const makeWCPredictions = catchAsync(async (req, res) => {
  const userid = req.userid;
  const predictions = req.body;
  if (!predictions) throw new ApiError(httpStatus.BAD_REQUEST, "Missing info!");
  let results = [];
  for (let i = 0; i < predictions.length; i++) {
    const { matchid, prediction } = predictions[i];
    if (isNaN(prediction)) {
      const remove = await WCPredictionsModel.removeWCPrediction(
        userid,
        matchid
      );
      results.push(remove);
    }
    const check = await WCPredictionsModel.checkIfExists(userid, matchid);
    if (check) {
      const result = await WCPredictionsModel.updateWCPrediction(
        check,
        prediction
      );
      results.push(result);
    } else {
      const result = await WCPredictionsModel.makeWCPrediction(
        userid,
        matchid,
        prediction
      );
...
const getWCPredictions = catchAsync(async (req, res) => {
  const userid = req.userid;
  const result = await WCPredictionsModel.getWCPredictions(userid);
  res.status(httpStatus.OK).send({ status: "success", result });
});
